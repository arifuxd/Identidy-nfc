...
export const Route = createFileRoute("/themes")({
  head: () => ({
    meta: [
      { title: "Themes — Identidy" },
      { name: "description", content: "Creator, Minimal, Designer, Developer, Gamer, Corporate and more. Browse the Identidy theme deck." },
      { property: "og:title", content: "Themes — Identidy" },
      { property: "og:description", content: "Browse the full Identidy theme deck." },
    ],
  }),
  component: Themes,
});
...
function ThemeCard({ theme, i, active, onClick }: { theme: Theme; i: number; active: boolean; onClick: () => void }) {
  // fan effect: rotate & offset by index around center
  return (
    <button
      onClick={onClick}
      data-cursor="expand"
      className="absolute left-1/2 top-0 origin-bottom transition-all duration-500"
      style={{
        transform: `translateX(-50%) rotate(${(i - active_center(i, active)) * 6}deg) translateY(${active ? -20 : 0}px) scale(${active ? 1.04 : 1})`,
        zIndex: active ? 20 : 10 - Math.abs(i),
      }}
    >
      <div
        className="w-[260px] rounded-2xl border border-hairline p-4 shadow-2xl"
        style={{
          background: theme.colors[0],
          color: theme.colors[1],
          boxShadow: "0 40px 60px -30px rgba(0,0,0,0.55), 0 8px 16px rgba(0,0,0,0.15)",
        }}
      >
        <div className="flex items-start justify-between">
          <div>
            <p className="text-[9px] uppercase tracking-[0.24em] opacity-60">{theme.category}</p>
            <p className="mt-3 font-display text-lg tracking-tight">{theme.name}</p>
          </div>
          <span className="h-2 w-2 rounded-full" style={{ background: theme.colors[2] }} />
        </div>
...
function active_center(_i: number, _active: boolean) { return 0; }
          ...
          <div className="relative mt-16 h-[380px] w-full max-w-[900px]">
            {THEMES.slice(0, 7).map((t, i) => {
              ...
              return (
                ...
                >
                  <div
                    className="w-[260px] rounded-2xl p-5"
                    style={{
      ...
      <section className="mx-auto max-w-[1440px] px-5 pb-24 sm:px-8">
        <div className="flex flex-wrap items-center justify-between gap-6 rounded-3xl border border-hairline bg-surface p-10">
          ...
          <div className="flex gap-2">
            <Link to="/demo" className="rounded-full border border-hairline px-5 py-3 text-sm">Try demo profile</Link>
            <Link to="/get-your-card" className="inline-flex items-center gap-2 rounded-full bg-foreground px-5 py-3 text-sm text-background">
