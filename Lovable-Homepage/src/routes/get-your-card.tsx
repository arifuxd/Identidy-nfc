...
export const Route = createFileRoute("/get-your-card")({
  head: () => ({
    meta: [
      { title: "Get Your Card — Identidy" },
      { name: "description", content: "Order your Identidy NFC business card. No payment now — we confirm your order and design within 24 hours." },
      { property: "og:title", content: "Get Your Card — Identidy" },
      { property: "og:description", content: "Order your Identidy card. We confirm within 24 hours." },
    ],
  }),
  component: GetYourCard,
});
        ...
        <div className="tap-in rounded-3xl border border-hairline bg-surface p-10 text-center sm:p-16">
          ...
          <ul className="mx-auto mt-10 grid max-w-md gap-3 text-left text-sm">
            {[
              ...
              <li key={s} className="flex items-start gap-3 rounded-xl border border-hairline p-4">
  ...
  return (
    <>
      <section className="mx-auto max-w-[1440px] px-5 pt-16 sm:px-8">
        ...
        <h1 className="tap-in-delay-1 mt-6 max-w-[22ch] text-balance font-display text-[clamp(2.25rem,5.5vw,5rem)] leading-[0.96] tracking-[-0.03em]">
          Get your card. <span className="italic text-ink-soft">No payment now.</span>
        </h1>
        <div className="tap-in-delay-2 mt-6 inline-flex items-start gap-3 rounded-2xl border border-hairline bg-surface p-4 text-sm">
          <Info size={16} className="mt-0.5 text-accent" />
          ...
          >
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-3">
                {TIERS.map((t) => (
                  <label
                    key={t.id}
                    className={[
                      "cursor-pointer rounded-2xl border p-4 transition-transform hover:-translate-y-0.5",
            ...
            <fieldset>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <Field label="Full name" required><input required className="i" placeholder="Ayaan Rahman" /></Field>
                <Field label="Email" required><input required type="email" className="i" placeholder="you@company.com" /></Field>
                <Field label="Phone / WhatsApp" required><input required className="i" placeholder="+880 1XXX ..." /></Field>
                <Field label="Company (optional)"><input className="i" placeholder="Studio Kagoj" /></Field>
              </div>
            </fieldset>
            ...
            <fieldset>
              ...
              <div className="mt-4 grid gap-3 sm:grid-cols-2">
                <Field label="Division" required>
                  <select required className="i">
                    {DIVISIONS.map((d) => <option key={d}>{d}</option>)}
                  </select>
                </Field>
                <Field label="District" required><input required className="i" placeholder="Dhaka" /></Field>
                <div className="sm:col-span-2">
                  <Field label="Full address" required><textarea required rows={3} className="i resize-none" placeholder="House, road, area, postcode" /></Field>
                </div>
              </div>
            </fieldset>
            ...
            <fieldset>
              ...
              <div className="mt-4 space-y-4">
                <Field label="Preferred theme">
                  <select className="i" value={theme} onChange={(e) => setTheme(e.target.value)}>
                    {THEMES.map((t) => <option key={t}>{t}</option>)}
                  </select>
                </Field>
                <Field label="Notes for the designer (optional)">
                  <textarea rows={3} className="i resize-none" placeholder="Colors, tone, links to include, anything we should know." />
                </Field>
              </div>
            </fieldset>
        ...
        <TapReveal delay={80} className="lg:col-span-4">
          <div className="sticky top-24 rounded-3xl border border-hairline bg-background p-6" style={{ boxShadow: "var(--shadow-card)" }}>
            ...
            <div className="mt-6 flex items-baseline justify-between border-t border-hairline pt-5">
              ...
              <span className="font-display text-2xl tracking-tight">{selected.price}</span>
            </div>
